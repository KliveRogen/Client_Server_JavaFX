#include "TR_VAPE_07H_AD3747.h"

TR_VAPE_07H_AD3747::TR_VAPE_07H_AD3747()
{
	// Расчетный тип блока
	...
	createParameter("as_dv_cu", "0");
	createParameter("al_dv_t", "0");
	createParameter("al_dv_cu", "0");

	// Сигналы блока
	createSignal("sm_t", Signal::ST_DOUBLE);
	createSignal("sm_cu", Signal::ST_DOUBLE);

	// Порты блока
    inh = createInputPort(0, "inh", "INFO");
    ins = createInputPort(1, "ins", "INFO");
...
void TR_VAPE_07H_AD3747::setDataNames()
{
    ...
    out3.push_back("Концентрация, г/л");
    outinfosen->setDataNames(out3);
}

bool TR_VAPE_07H_AD3747::init(std::string &error, double h)
{
    //инициализации
...
bool TR_VAPE_07H_AD3747::process(double t, double h, std::string &error)
{
    //инициализации
        //перем
        double  CCin[el],buf,qheat,fin,tin,thout,tout;
        //входы
        fin = ins->getInput()[0];
...
ICalcElement *Create()
{
    return new TR_VAPE_07H_AD3747();
}

void Release(ICalcElement *block)
{
    if(block->type() == "TR_VAPE_07H_AD3747") delete block;
}

std::string Type()
{
    return "TR_VAPE_07H_AD3747";
}
