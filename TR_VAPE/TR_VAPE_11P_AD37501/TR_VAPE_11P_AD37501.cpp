#include "TR_VAPE_11P_AD37501.h"

TR_VAPE_11P_AD37501::TR_VAPE_11P_AD37501()
{
	// Расчетный тип блока
    ...
    createParameter("pc_ppow", "0");        //1
    createParameter("as_dv_p", "0");        //2
    createParameter("as_dv_i", "0");        //3
    createParameter("as_dv_f", "0");        //4
    createParameter("as_dl_ind", "0");      //5
    createParameter("as_av_ppow", "0");     //6
    createParameter("al_dv_p", "0");        //7
    createParameter("al_dv_i", "0");        //8
    createParameter("al_dv_f", "0");        //9
    createParameter("al_dl_ind", "0");      //10
    createParameter("al_av_ppow", "0");     //11

	// Сигналы блока
    createSignal("sm_p", Signal::ST_DOUBLE);        //0
    createSignal("sm_i", Signal::ST_DOUBLE);        //1
    createSignal("sm_f", Signal::ST_DOUBLE);        //2
    createSignal("sm_ind", Signal::ST_DOUBLE);      //3
    createSignal("sc_ppow", Signal::ST_DOUBLE);     //4
...
void TR_VAPE_11P_AD37501::setDataNames()
{
...
bool TR_VAPE_11P_AD37501::init(std::string &error, double h)
{
    //инициализации
        for(int i=0;i<el;i++)
            MM[i] = 0;
...
bool TR_VAPE_11P_AD37501::process(double t, double h, std::string &error)
{
    //инициализации
        //перем
    ...
    //выходы
        ...
        if(eflg!=0)
        {
            error = "Ошибка блока 11)АД-3750/1(нас.):\n";
            if(eflg==1)
            {
...
ICalcElement *Create()
{
    return new TR_VAPE_11P_AD37501();
}

void Release(ICalcElement *block)
{
    if(block->type() == "TR_VAPE_11P_AD37501") delete block;
}

std::string Type()
{
    return "TR_VAPE_11P_AD37501";
}
