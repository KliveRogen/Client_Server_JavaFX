#include "TR_EXTRACTOR.h"

#include <math.h>

TR_EXTRACTOR::TR_EXTRACTOR()
{
...
std::string Type()
{
    return "TR_HEADEXTRACTOR";
}


bool TR_EXTRACTOR::process(double t, double h, std::string &error)
{
    if(NeedInitDelays) {
    ...
    // Пр.201
    if(IP_F201->hasConnection()) {
        F201.Q = IP_F201->getInput()[0];
        F201.T = IP_F201->getInput()[1];
        F201.C = IP_F201->getInput()[2];
    }
    else {
        F201.Q = 0.00;
        F201.T = 0.00;
    ...
    // Пр.200
    if(IP_F200->hasConnection()) {
        F200.Q = IP_F200->getInput()[0];
        F200.T = IP_F200->getInput()[1];
        F200.C = IP_F200->getInput()[2];
    }
    else {
        F200.Q = 0.00;
        F200.T = 0.00;
    ...
    // Пр.222
    if(IP_F222->hasConnection()) {
        F222.Q = IP_F222->getInput()[0];
        F222.T = IP_F222->getInput()[1];
        F222.C = IP_F222->getInput()[2];
    }
    else {
        F222.Q = 0.00;
        F222.T = 0.00;
...
bool TR_EXTRACTOR::init(std::string &error, double h)
{
    ...
    OP_F210->setDataNames(dn);
    OP_F213->setDataNames(dn);
...
void TR_EXTRACTOR::createPorts()
{
    // Входные порты
    IP_Mag      = createInputPort(0, "Магистраль", "INFO");
    IP_ExtPump  = createInputPort(1, "Выносной насос", "INFO");
    IP_F200     = createInputPort(2, "Вх. пр. 200", "INFO");
    IP_F201     = createInputPort(3, "Вх. пр. 201", "INFO");
    IP_F222     = createInputPort(4, "Вх. пр. 222", "INFO");
    IP_V07      = createInputPort(5, "Кл. 07/1", "INFO");
    IP_V06      = createInputPort(6, "Кл. 06/1", "INFO");
    IP_V42     = createInputPort(7, "Кл. 42/2", "INFO");
    IP_VTank    = createInputPort(8, "Кл. емкости", "INFO");
    IP_PumpTank = createInputPort(9, "Насос", "INFO");
    IP_Debug    = createInputPort(10, "Отладочный вход", "INFO");

    // Выходные порты
    OP_F210  = createOutputPort(11, "Вых. пр. 210", "INFO");
    OP_F213  = createOutputPort(12, "Вых. пр. 213", "INFO");
    OP_Debug = createOutputPort(13, "Отладочный выход", "INFO");
}
