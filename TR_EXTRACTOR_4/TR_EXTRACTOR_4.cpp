#include "TR_EXTRACTOR_4.h"

#include <math.h>

TR_EXTRACTOR_4::TR_EXTRACTOR_4()
{
...
bool TR_EXTRACTOR_4::process(double t, double h, std::string &error)
{
    if(NeedInitDelays) {
    ...
    // Пр.59
    if(IP_F59->hasConnection()) {
        F59.Q = IP_F59->getInput()[0];
        F59.T = IP_F59->getInput()[1];
        F59.C = IP_F59->getInput()[2];
    }
    else {
        F59.Q = 0.00;
        F59.T = 0.00;
    ...
    // Пр.230
    if(IP_F230->hasConnection()) {
        F230.Q = IP_F230->getInput()[0];
        F230.T = IP_F230->getInput()[1];
        F230.C = IP_F230->getInput()[2];
    }
    else {
        F230.Q = 0.00;
        F230.T = 0.00;
...
bool TR_EXTRACTOR_4::init(std::string &error, double h)
{
    ...
    OP_F200->setDataNames(dn);
    OP_F247->setDataNames(dn);
...
void TR_EXTRACTOR_4::createPorts()
{
    // Входные порты
    IP_Mag      = createInputPort(0, "Магистраль", "INFO");
    IP_ExtPump  = createInputPort(1, "Выносной насос", "INFO");
    IP_F230     = createInputPort(2, "Вх. пр. 230", "INFO");
    IP_F59     = createInputPort(3, "Вх. пр. 59", "INFO");
    ...
    // Выходные порты
    OP_F200  = createOutputPort(10, "Вых. пр. 200", "INFO");
    OP_F247  = createOutputPort(11, "Вых. пр. 247", "INFO");
