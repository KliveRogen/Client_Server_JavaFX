#include "TR_REEXTRACTOR.h"

#include <math.h>

TR_REEXTRACTOR::TR_REEXTRACTOR()
{
...
bool TR_REEXTRACTOR::process(double t, double h, std::string &error)
{
    if(NeedInitDelays) {
    ...
    // Пр.269
    if(IP_F269->hasConnection()) {
        F269.Q = IP_F269->getInput()[0];
        F269.T = IP_F269->getInput()[1];
        F269.C = IP_F269->getInput()[2];
    }
    else {
        F269.Q = 0.00;
        F269.T = 0.00;
    ...
    // Пр.220
    if(IP_F220->hasConnection()) {
        F220.Q = IP_F220->getInput()[0];
        F220.T = IP_F220->getInput()[1];
        F220.C = IP_F220->getInput()[2];
    }
    else {
        F220.Q = 0.00;
        F220.T = 0.00;
...
bool TR_REEXTRACTOR::init(std::string &error, double h)
{
    ...
    OP_F230->setDataNames(dn);
    OP_F235->setDataNames(dn);
...
void TR_REEXTRACTOR::createPorts()
{
    // Входные порты
    IP_Mag      = createInputPort(0, "Магистраль", "INFO");
    IP_ExtPump  = createInputPort(1, "Выносной насос", "INFO");
    IP_F220     = createInputPort(2, "Вх. пр. 220", "INFO");
    IP_F269     = createInputPort(3, "Вх. пр. 269", "INFO");
    ...
    // Выходные порты
    OP_F230  = createOutputPort(10, "Вых. пр. 230", "INFO");
    OP_F235  = createOutputPort(11, "Вых. пр. 235", "INFO");
