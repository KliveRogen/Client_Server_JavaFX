#include "HYDR_REGVALVE.h"
#include "math.h"

HYDR_REGVALVE::HYDR_REGVALVE()
{
	// Расчетный тип блока
...
void HYDR_REGVALVE::setDataNames()
{
...
bool HYDR_REGVALVE::init(std::string &error, double h)
{
	setDataNames();
    return true;
}

bool HYDR_REGVALVE::process(double t, double h, std::string &error)
{
...
ICalcElement *Create()
{
    return new HYDR_REGVALVE();
}

void Release(ICalcElement *block)
{
    if(block->type() == "HYDR_REGVALVE") delete block;
}

std::string Type()
{
    return "HYDR_REGVALVE";
}
